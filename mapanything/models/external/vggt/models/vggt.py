...
class VGGT(nn.Module, PyTorchModelHubMixin):
    ...
    def forward(
        ...
        # Compute the set of aggregator output indices actually needed by the heads.
        # CameraHead uses [-1] (= depth-1), DPT heads use intermediate_layer_idx.
        # Only these indices are materialized; others are dropped to save memory.
        retain_indices = set(self.depth_head.intermediate_layer_idx)
        retain_indices.add(self.aggregator.depth - 1)  # for camera_head's [-1]

        aggregated_tokens_list, patch_start_idx = self.aggregator(
            images, retain_indices=retain_indices
        )
